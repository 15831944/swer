...
CProgram::~CProgram()
{
	...
	if (wxGetApp().m_program == this)
	{
		wxGetApp().m_program = NULL;
	}
}
...
const wxBitmap &CProgram::GetIcon()
{
	...
	if(icon == NULL)icon = new wxBitmap(wxImage(wxGetApp().GetResFolder() + _T("/icons/program.png")));
	return *icon;
}
...
void CProgram::WriteXML(TiXmlNode *root)
{
	...
	element->SetAttribute( "program", wxGetApp().m_program_canvas->m_textCtrl->GetValue().utf8_str());
...
HeeksObj* CProgram::ReadFromXMLElement(TiXmlElement* pElem)
{
	...
	if (wxGetApp().m_program == NULL)
	{
	    wxGetApp().m_program = new_object;
	}
	...
	for(TiXmlAttribute* a = pElem->FirstAttribute(); a; a = a->Next())
	{
		...
		else if(name == "program"){wxGetApp().m_program_canvas->m_textCtrl->SetValue(Ctt(a->Value()));}
...
{
	CPattern* pattern = (CPattern*)wxGetApp().GetIDObject(PatternType, p);
	if(pattern)
	{
...
void ApplySurfaceToText(Python &python, CSurface* surface, std::set<CSurface*> &surfaces_written)
{
	if(surfaces_written.find(surface) == surfaces_written.end())
	{
		...
		{
			HeeksObj* object = wxGetApp().GetIDObject(SolidType, *It);
		...
		//write stl file
		wxGetApp().SaveSTLFile(solids, filepath.GetFullPath(), 0.01);

		python << _T("stl") << (int)(surface->m_id) << _T(" = ocl_funcs.STLSurfFromFile(") << PythonString(filepath.GetFullPath()) << _T(")\n");
	}

	python << _T("attach.units = ") << wxGetApp().m_program->m_units << _T("\n");
	...
	wxGetApp().m_attached_to_surface = surface;
}
...
Python CProgram::RewritePythonProgram()
{
	...
	wxGetApp().m_program_canvas->m_textCtrl->Clear();
	wxGetApp().m_attached_to_surface = NULL;
	CSurface::number_for_stl_file = 1;
	wxGetApp().m_tool_number = 0;

	// call any OnRewritePython functions from other plugins
	for(std::list< void(*)() >::iterator It = wxGetApp().m_OnRewritePython_list.begin(); It != wxGetApp().m_OnRewritePython_list.end(); It++)
	{
...
#ifdef CMAKE_UNIX
	#ifdef RUNINPLACE
	        python << _T("sys.path.insert(0,'") << wxGetApp().GetResFolder() << _T("/')\n");
	#else
...
#endif
#endif
	python << _T("sys.path.insert(0,") << PythonString(wxGetApp().GetResFolder()) << _T(")\n");
#endif
	...
	{
#ifdef WIN32
	python << _T("sys.path.insert(0,") << PythonString(wxGetApp().GetResFolder() + (wxGetApp().m_use_Clipper_not_Boolean ? _T("/Clipper"):_T("/Boolean"))) << _T(")\n");
#endif
	...
	for (OperationsMap_t::const_iterator l_itOperation = operations.begin(); l_itOperation != operations.end(); l_itOperation++)
	{
		...
		if(COperations::IsAnOperation(object->GetType()))
		{
			...
			if(op->m_active)
			{
				CSurface* surface = (CSurface*)wxGetApp().GetIDObject(SurfaceType, op->m_surface);
	...
	wxGetApp().m_program_canvas->m_textCtrl->AppendText(python);
	if (python.Length() > wxGetApp().m_program_canvas->m_textCtrl->GetValue().Length())
	{
		...
		wxGetApp().m_program_canvas->m_textCtrl->Clear();
		wxGetApp().m_program_canvas->m_textCtrl->AppendText(_("The Python program is too long \n"));
		wxGetApp().m_program_canvas->m_textCtrl->AppendText(_("to display in this window.\n"));
		wxGetApp().m_program_canvas->m_textCtrl->AppendText(_("Please edit the python program directly at \n"));
		wxGetApp().m_program_canvas->m_textCtrl->AppendText(file_str.GetFullPath());
	}
...
void CProgram::GetMachines(std::vector<CMachine> &machines)
{
	...
	if(machines_file.Len() == 0)machines_file = wxGetApp().GetResFolder() + _T("/nc/machines.xml");
...
{
	wxString script_op_file = wxGetApp().GetResourceFilename(wxT("script_ops.xml"));
...
wxString CProgram::GetOutputFileName() const
{
	...
	{
		if (wxGetApp().GetFileFullPath())
		{
			wxString path(wxGetApp().GetFileFullPath());
			int offset = -1;
...
{
	ProgramDlg dlg(wxGetApp().m_frame, (CProgram*)object);
